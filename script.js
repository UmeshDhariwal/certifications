...
// Container for displaying meals
const mealsDiv = document.getElementById('meals-div'); 
// Button for displaying random image
const randomButton = document.getElementById('random-image'); 
// Button for displaying favorite meals
const myFavMeal = document.getElementById('my-favourite-meals'); 

let favArray = []; // Array to store favorite meal IDs
let URL; // URL for API requests

// Check if favArray exists in localStorage, otherwise initialize it
...
async function moreDetails() {
  ...
  div.innerHTML = `
    ...
    <a href="${meals.strYoutube}" target="_blank"><button type="button" class='border-circle more-details' id='${meals.idMeal}'>Watch Video</button></a>`;
...
function toggleFavorites(event) {
  ...
  let index = favArray.indexOf(this.id);
  if (index == -1) {
  ...
  try {
    ...
    for (let meals of data.meals) {
      ...
      div.innerHTML = `
        ...
        ${
          favArray.includes(meals.idMeal) ? `<a href="" class='favourite clicked' id='${meals.idMeal}'><i class="fa-sharp fa-solid fa-heart"></i></a>` : `<a href="" class='favourite' id='${meals.idMeal}'><i class="fa-sharp fa-solid fa-heart"></i></a>`
        }`;
...
myFavMeal.addEventListener('click', displayFavoriteMeals); // Favorite meals button click event
